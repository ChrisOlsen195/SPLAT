 ...
 */
package noncentrals.JDistr_MathyStuff;
...
public class MathFunctions {
	...
	public static final double lmvgammafn(double a, int p) {
		...
		return sum + p*(p-1)/4.0 * Constants.M_LOG_PI;
	}
	...
	public static final double lgammafn_sign(double x, int []sgn)
	{
		...
		if (x > 0) { /* i.e. y = x > 10 */
			...
			return Constants.M_LN_SQRT_2PI + (x - 0.5) * log(x) - x + lgammacor(x);
		}
		/* else: x < -10; y = -x */
		sinpiy = abs(sinpi(y));
		...
		ans = Constants.M_LN_SQRT_PId2 + (x - 0.5) * log(y) - x - log(sinpiy) - lgammacor(y);
		...
		if(abs((x - trunc(x - 0.5)) * ans / x) < dxrel) {
			...
			if (Debug.WARNING_AS_ERROR) throw new RuntimeException("lgamma precision error!");
		}
		return ans;
	}
	...
	public static final double stirlerr(double n)
	{
		...
		if (n <= 15.0) {
			...
			return(lgammafn(n + 1.) - (n + 0.5)*log(n) + n - Constants.M_LN_SQRT_2PI);
		}
		nn = n*n;
	...
	public static final double logspace_sub (double logx, double logy) {
		logy = logy - logx;
	    return logx + ((logy) > -Constants.M_LN2 ? log(-expm1(logy)) : log1p(-exp(logy)));
	}
	...
	public static final double[] bratio(double a, double b, double x, double y, boolean log_p)
	{
		...
		double eps = 2. * Constants.DBL_EPSILON; /* == DBL_EPSILON (in R, Rmath) */
		...
		if(min(a,b) <= 1.) {
			...
			if (b0 < min(eps, eps * a0)) { /* L80: */
				...
				w1 = log_p ? ((w) > -Constants.M_LN2 ? log(-expm1(w)) : log1p(-exp(w))) : 0.5 - w + 0.5;
				// goto L_end;
				if (do_swap) { /* swap */
			...
			if (kase == 0 || kase == 131) {
				...
				if (w1 == 0 || (0 < w1 && w1 < 1e-322)) { // w1=0 or very close:
					...
					if(log_p) {
						w = ((w1) > -Constants.M_LN2 ? log(-expm1(w1)) : log1p(-exp(w1)));
					} else {
						w  = /* 1 - exp(*w1) */ -expm1(w1);
						w1 = exp(w1);
					}
		...
		switch(kase)
		{
			...
			case 110:
				...
				w  = log_p ? ((w1) > -Constants.M_LN2 ? log(-expm1(w1)) : log1p(-exp(w1))) : 0.5 - w1 + 0.5;
				// goto L_end_after_log;
				break;
			case 120:
	...
	public static final double bpser(double a, double b, double x, double eps, boolean log_p)
	{
		...
		if(log_p) {
			if (a*sum > -1.0) ans += log1p(a * sum);
				...
				if (ans > Double.NEGATIVE_INFINITY && Debug.WARNING_AS_ERROR)
	...
	public static final double bfrac(double a, double b, double x, double y, double lambda, double eps, boolean log_p)
	{
	   ...
	   -----------------------------------------------------------------------*/
		...
		if (!log_p && brc == 0.) { /* already underflowed to 0 */
			if (Debug.WARNING_AS_ERROR) throw new PrecisionException(" --> brcomp(a,b,x,y) underflowed to 0.\n", 0);
			return 0.;
		}
		...
		if(n >= 10000 && abs(r - r0) > eps * r && Debug.WARNING_AS_ERROR) {
	...
	public static final double brcomp(double a, double b, double x, double y, boolean log_p)
	{
		...
		return(log_p
			? -Constants.M_LN_SQRT_2PI + .5*log(b * x0) + z - bcorr(a,b)
			: Constants.M_1_SQRT_2PI * sqrt(b * x0) * z * exp(-bcorr(a, b)));
	} /* brcomp */
	...
	public static final double brcmp1(int mu, double a, double b, double x, double y, boolean give_log)
	{
		...
		if (a0 < 8.0) {
			...
			return give_log
				? log(Constants.M_1_SQRT_2PI)+ (log(b) + lx0)/2. + z      - bcorr(a, b)
				:     Constants.M_1_SQRT_2PI * sqrt(b * x0)      * z * exp(-bcorr(a, b));
		}
	} /* brcmp1 */
	...
	public static final double grat_r(double a, double x, double log_r, double eps)
	{
		if (a * x == 0.0) { /* L130: */
			...
			if (x < 0.25) {
				...
				double sx = sqrt(x),
						q_r = erfc1(1, sx)/sx * Constants.M_SQRT_PI;
				return q_r;
			}
	...
	public static final double exparg(int l)
	{
		...
		int m = l == 0 ? Constants.DBL_MAX_EXP : Constants.DBL_MIN_EXP - 1;
		return m * lnb * .99999;
	} /* exparg */
	...
	public static final double psi(double x)
	{
		...
		d2 = 0.5 / (0.5*Constants.DBL_EPSILON);
	...
	public static final double betaln(double a0, double b0)
	{
		...
		if (a < 8.0) {
			...
			v = b * alnrel(h);
			if (u > v)
				return log(b) * -0.5 + Constants.M_LN_SQRT_2PI + w - v - u;
			else
				return log(b) * -0.5 + Constants.M_LN_SQRT_2PI + w - u - v;
		}
	} /* betaln */
	...
	public static final double gamma_cody(double x) {
		...
		if (y < Constants.DBL_EPSILON) {
			...
			if (y <= xbig) {
				...
				sum = sum / y - y + Constants.M_LN_SQRT_2PI; //sqrtpi;
				sum += (y - .5) * log(y);
				res = exp(sum);
			} else {
				return(Double.POSITIVE_INFINITY);
	...
	public static final double pd_upper_series (double x, double y, boolean log_p)
	{
		...
		do {
			...
			term *= x / y;
			sum += term;
		} while (term > sum * Constants.DBL_EPSILON);
	...
	public static final double pd_lower_cf (double y, double d)
	{
		...
		if(abs(y - 1) < abs(d) * Constants.DBL_EPSILON) { /* includes y < d = Inf */
			return (f0);
		}
		...
		while (i < max_it) {
			...
			if (b2 != 0) {
				...
				if (abs (f - of) <= Constants.DBL_EPSILON * max(f0, abs(f))) {
					return f;
				}
				of = f;
			}
	...
	public static final double pd_lower_series (double lambda, double y)
	{
		...
		while (y >= 1 && term > sum * Constants.DBL_EPSILON) {
			term *= y / lambda;
			sum += term;
			y--;
		}
	...
	public static final double _expm1(double x)
	{
	    ...
	    if (a < Constants.DBL_EPSILON) return x;
	...
	public static final double _log1p(double x)
	{
		...
		if(abs(x) < .5 * Constants.DBL_EPSILON)
		    return x;
		...
		/* answer less than half precision because x too near -1 */
			...
			if (Debug.WARNING_AS_ERROR) throw new RuntimeException("Precision loss warning at log1p");
