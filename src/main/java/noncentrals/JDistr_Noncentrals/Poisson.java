...
public class Poisson extends GenericDistribution {
	...
	public static final double density_raw(double x, double lambda, boolean give_log) {
		...
		if (x <= lambda * Constants.DBL_MIN) return(give_log ? -lambda : exp(-lambda) );
		if (lambda < x * Constants.DBL_MIN) {
			if (MathFunctions.isInfinite(x))
				 return (give_log ? Double.NEGATIVE_INFINITY : 0.);
			x = -lambda + x*log(lambda) -MathFunctions.lgammafn(x+1);
	...
	public static final double density(double x, double lambda, boolean give_log) {
		...
		if (MathFunctions.isNonInt(x)) return (give_log ? Double.NEGATIVE_INFINITY : 0.); // Non integer
	...
	public static final double quantile(double p, double lambda, boolean lower_tail, boolean log_p) {
		...
		if (p + 1.01 * Constants.DBL_EPSILON >= 1.) return Double.POSITIVE_INFINITY;
	...
	public static final double random(double mu, RandomEngine random, RandomState state) {
		...
		if (new_big_mu || mu != state.muprev2) {
			 ...
			 */
			state.muprev2 = mu;
			state.omega = Constants.M_1_SQRT_2PI / state.s;
		...
		for(;;) {
			...
			if (pois < 10) { /* use factorials from table fact[] */
				...
				py = Constants.M_1_SQRT_2PI / sqrt(fk);
			}
