...
import noncentrals.JDistr_MathyStuff.Constants;
import noncentrals.JDistr_MathyStuff.MathFunctions;
...
public class NonCentralT extends GenericDistribution {
	...
	public static final double density(double x, double df, double ncp, boolean give_log) {
		...
		if(ncp == 0.0) return TDistribution.density(x, df, give_log);
		...
		if (abs(x) > sqrt(df * Constants.DBL_EPSILON)) {
			u = log(df) - log(abs(x)) +
			...
			u = MathFunctions.lgammafn((df+1)/2) - MathFunctions.lgammafn(df/2)
					- (Constants.M_LN_SQRT_PI + 0.5*(log(df) + ncp*ncp));
		}
	...
	public static final double cumulative(double t, double df, double ncp, boolean lower_tail, boolean log_p) {
		...
		if (df <= 0.0) return Double.NaN;
		if(ncp == 0.0) return TDistribution.cumulative(t, df, lower_tail, log_p);
		...
		if (df > 4e5 || del*del > 2 * Constants.M_LN2*(-(Constants.DBL_MIN_EXP))) {
		...
		if (x > 0.) {/* <==>  t != 0 */
			...
			if(p == 0.) { /* underflow! */
				...
				if (Debug.WARNING_AS_ERROR) throw new PrecisionException("Underflow error in NonCentralT.cumulative", lower_tail ? (log_p ? Double.NEGATIVE_INFINITY : 0.) : (log_p ? 0. : 1.));
			...
			rxb = pow(rxb, b);
			albeta = Constants.M_LN_SQRT_PI + MathFunctions.lgammafn(b) - MathFunctions.lgammafn(.5 + b);
			xodd = Beta.cumulative(x, a, b, /*lower*/true, /*log_p*/false);
			godd = 2. * rxb * exp(a * log(x) - albeta);
			tnc = b * x;
			xeven = (tnc < Constants.DBL_EPSILON) ? tnc : 1. - rxb;
			geven = tnc * rxb;
			tnc = p * xodd + q * xeven;
			...
			for(it = 1; it <= itrmax; it++) {
				...
				if(s < -1.e-10) { /* happens e.g. for (t,df,ncp)=(40,10,38.5), after 799 it.*/
					...
					if (Debug.WARNING_AS_ERROR && exception == null) exception = "Underflow error in NonCentralT.cumulative";
					//goto finis;
					break;
				}
		...
		if(tnc > 1 - 1e-10 && lower_tail) {
			...
			if (Debug.WARNING_AS_ERROR && exception == null) exception = "Precision error in final section of NonCentralT.cumulative";
		}
	...
	public static final double quantile(double p, double df, double ncp, boolean lower_tail, boolean log_p) {
		...
		if(ncp == 0.0 && df >= 1.0) return TDistribution.quantile(p, df, lower_tail, log_p);
		...
		for(ux = max(1., ncp);
				ux < Constants.DBL_MAX && cumulative(ux, df, ncp, true, false) < pp;
				ux *= 2);
		pp = p * (1 - Eps);
		for(lx = min(-1., -ncp);
				lx > -Constants.DBL_MAX && cumulative(lx, df, ncp, true, false) > pp;
				lx *= 2);
	...
	public static final double random(double df, double ncp, RandomEngine random) {
		if (ncp == 0)
			return TDistribution.random(df, random);
