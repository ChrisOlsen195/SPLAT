...
public class TDistribution extends GenericDistribution {
	public static final double density(double x, double n, boolean give_log) {
		...
		double u, ax = abs(x),
				t = -MathFunctions.bd0(n/2.,(n+1)/2.) + MathFunctions.stirlerr((n+1)/2.) - MathFunctions.stirlerr(n/2.),
				x2n = x*x/n, // in  [0, Inf]
				l_x2n; // := log(sqrt(1 + x2n)) = log(1 + x2n)/2
		boolean lrg_x2n =  (x2n > 1./Constants.DBL_EPSILON);
		if (lrg_x2n) { // large x^2/n :
			...
			l_x2n = log1p(x2n)/2.;
			u = -MathFunctions.bd0(n/2.,(n+x*x)/2.) + x*x/2.;
		}

		if(give_log)
			return t-u - (Constants.M_LN_SQRT_2PI + l_x2n);

		// else :  if(lrg_x2n) : sqrt(1 + 1/x2n) ='= sqrt(1) = 1
		double I_sqrt_ = (lrg_x2n ? sqrt(n)/ax : exp(-l_x2n));
		return exp(t-u) * Constants.M_1_SQRT_2PI * I_sqrt_;
	}
	...
	public static final double cumulative(double x, double n, boolean lower_tail, boolean log_p) {
		...
		if(MathFunctions.isInfinite(x))
		   ...
		   with z = 1/nx,  a = n/2,  b= 1/2 :
				 */
                    double lval;
                    lval = -0.5*n*(2*log(abs(x)) - log(n)) - MathFunctions.lbeta(0.5*n, 0.5) - log(0.5*n);
                ...
                if(log_p) {
                    ...
                    else return val - Constants.M_LN2; /* = log(.5* pbeta(....)) */
                }
                else {
                    val /= 2.;
                    //return R_D_Cval(val);
	...
	public static final double quantile(double p, double ndf, boolean lower_tail, boolean log_p) {
		...
		if (ndf < 1) { /* based on qnt */
                    ...
                    if(p > 1 - Constants.DBL_EPSILON) return Double.POSITIVE_INFINITY;
                    pp = min(1 - Constants.DBL_EPSILON, p * (1 + Eps));
                    for(ux = 1.; ux < Constants.DBL_MAX && cumulative(ux, ndf, true, false) < pp; ux *= 2);
                    pp = p * (1 - Eps);
                    for(lx =-1.; lx > -Constants.DBL_MAX && cumulative(lx, ndf, true, false) > pp; lx *= 2);
                    ...
                    if(iter >= 1000) 
                        return Double.NaN; //ML_ERROR(ME_PRECISION, "qt");
		 ...
		 */
		if (ndf > 1e20) 
                    return Normal.quantile(p, 0., 1., lower_tail, log_p);
		...
		if (abs(ndf - 2) < eps) {	/* df ~= 2 */
                    if(P > Constants.DBL_MIN) {
                        if(3* P < Constants.DBL_EPSILON) /* P ~= 0 */
                                q = 1.0 / sqrt(P);
                        else if (P > 0.9)	   /* P ~= 1 */
                                q = (1 - P) * sqrt(2 /(P * (2 - P)));
                        else /* eps/3 <= P <= 0.9 */
                        ...
                        if(log_p)
                            q = is_neg_lower ? exp(- p/2) / Constants.M_SQRT_2 : 1/sqrt(-expm1(p));
                        else
                    ...
                    if(P > 0)
                        q = 1/tan(P * Constants.M_PI_2);/* == - tan((P+1) * M_PI_2) -- suffers for P ~= 0 */

                    else { /* P = 0, but maybe = 2*exp(p) ! */
                        if(log_p) /* 1/tan(e) ~ 1/e */
                            q = is_neg_lower ? Constants.M_1_PI * exp(-p) : -1./(PI * expm1(p));
                        else
                            q = Double.POSITIVE_INFINITY;
                    }
		}
			...
			double x = 0., y = 0, log_P2 = 0./* -Wall */,
					...
					d = ((94.5 / (b + c) - 3) / b + 1) * sqrt(a * Constants.M_PI_2) * ndf;

			boolean P_ok1 = P > Constants.DBL_MIN || !log_p,  P_ok = P_ok1;
			if(P_ok1) {
				y = pow(d * P, 2 / ndf);
				P_ok = (y >= Constants.DBL_EPSILON);
			}
			if(!P_ok) { /* log_p && P very small */
				// log_P2 = is_neg_lower ? R_D_log(p) : R_D_LExp(p); /* == log(P / 2) */
				log_P2 = is_neg_lower ? (log_p ? (p) : log(p)) : (log_p ? ((p) > -Constants.M_LN2 ? log(-expm1(p)) : log1p(-exp(p))) : log1p(-p));
				x = (log(d) + Constants.M_LN2 + log_P2) / ndf;
				y = exp(2 * x);
			}
			...
			if ((ndf < 2.1 && P > 0.5) || y > 0.05 + a) { /* P > P0(df) */
				...
				y = expm1(a * y * y);
				q = sqrt(ndf * y);
			} else if(!P_ok && x < - Constants.M_LN2 * Constants.DBL_MANT_DIG) {/* 0.5* log(DBL_EPSILON) */
