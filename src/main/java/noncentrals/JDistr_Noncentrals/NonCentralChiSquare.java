...
public class NonCentralChiSquare extends GenericDistribution {
	static final double _dbl_min_exp = Constants.M_LN2 * Constants.DBL_MIN_EXP;
	/*
	...
	public static final double cumulative_raw(double x, double f, double theta, double errmax, double reltol, int itrmax, boolean lower_tail, boolean log_p) {
		...
		final double _dbl_min_exp = Constants.M_LN2 * Constants.DBL_MIN_EXP;
		...
		if (x <= 0.) {
			if(x == 0. && f == 0.) {
				...
				return lower_tail ? (log_p	?  (_L)	 : exp(_L)) : (log_p ? ((_L) > -Constants.M_LN2 ? log(-expm1(_L)) : log1p(-exp(_L))) : -expm1(_L));
			}
			/* x < 0  or {x==0, f > 0} */
			// return R_DT_0;
		...
		if(theta < 80) { /* use 110 for Inf, as ppois(110, 80/2, lower.tail=false) is 2e-20 */
			...
			if(lower_tail && f > 0. &&
				log(x) < Constants.M_LN2 + 2/f*(MathFunctions.lgammafn(f/2. + 1) + _dbl_min_exp)) {
				...
				for(i = 0; i < 110;  pr += log(lambda) - log(++i)) {
					...
					sum = MathFunctions.logspace_add(sum, pr + ChiSquare.cumulative(x, f+2*i, lower_tail, true));
		...
		if(f2 * Constants.DBL_EPSILON > 0.125 && /* very large f and x ~= f: probably needs */
				abs(t = x2 - f2) <         /* another algorithm anyway */ // TODO long abs
				sqrt(Constants.DBL_EPSILON) * f2) {
			/* evade cancellation error */
			/* t = exp((1 - t)*(2 - t/(f2 + 1))) / sqrt(2*M_PI*(f2 + 1));*/
			lt = (1 - t)*(2 - t/(f2 + 1)) - Constants.M_LN_SQRT_2PI - 0.5 * log(f2 + 1);
		}
		else {
			/* Usual case 2: careful not to overflow .. : */
			lt = f2*log(x2) -x2 - MathFunctions.lgammafn(f2 + 1);
		}
		...
		if (is_it) {
			...
			if (Debug.WARNING_AS_ERROR) throw new PrecisionException("NonCentralChiSquare.cumulative_raw non-convergence error", (lower_tail ? (log_p	? log(ans) : (ans))  : (log_p ? log1p(-(ans)) : (0.5 - (ans) + 0.5))));
		}
		//return R_DT_val(ans);
	...
	public static final double cumulative(double x, double df, double ncp, boolean lower_tail, boolean log_p) {
		...
		if (Debug.WARNING_AS_ERROR) {
			try {
				ans = cumulative_raw(x, df, ncp, 1e-12, 8 * Constants.DBL_EPSILON, 1000000, lower_tail, log_p);				
		...
		if (Debug.WARNING_AS_ERROR) {
			try {
				ans = cumulative_raw(x, df, ncp, 1e-12, 8 * Constants.DBL_EPSILON, 1000000, !lower_tail, false);
			} catch (PrecisionException e) {
	...
	public static final double quantile(double p, double df, double ncp, boolean lower_tail, boolean log_p) {
	    ...
	    final double racc = 4 * Constants.DBL_EPSILON;
		...
		// pp = R_D_qIv(p);
		pp = log_p	? exp(p) : p;
		if(pp > 1 - Constants.DBL_EPSILON) return lower_tail ? Double.POSITIVE_INFINITY : 0.0;
		...
		if(!lower_tail && ncp >= 80) {
			...
			if(pp < 1e-10) {
				...
				if (Debug.WARNING_AS_ERROR) throw new RuntimeException("Precision loss detected in NonCentralChiSquare.quantile");
			}
	    ...
	    pp = min(1 - Constants.DBL_EPSILON, p * (1 + Eps));
	    if(lower_tail) {
	        for(; ux < Constants.DBL_MAX &&
			cumulative_raw(ux, df, ncp, Eps, rEps, 10000, true, false) < pp;
		    ux *= 2);
		pp = p * (1 - Eps);
	        for(lx = min(ux0, Constants.DBL_MAX);
		    lx > Constants.DBL_MIN &&
