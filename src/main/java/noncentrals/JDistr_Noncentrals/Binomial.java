...
public class Binomial extends GenericDistribution {
	...
	public static final double density_raw(double x, double n, double p, double q, boolean log_p)
	{
		...
		if (x == 0) {
			...
			lc = (p < 0.1) ? -MathFunctions.bd0(n,n*q) - n*p : n*log(q);
			//return( R_D_exp(lc) );
			return (log_p ? (lc) : exp(lc));
		}
		if (x == n) {
			lc = (q < 0.1) ? -MathFunctions.bd0(n,n*p) - n*q : n*log(p);
			return (log_p ? (lc) : exp(lc));
		}
		...
		lc = MathFunctions.stirlerr(n) - MathFunctions.stirlerr(x) - MathFunctions.stirlerr(n-x) - MathFunctions.bd0(x,n*p) - MathFunctions.bd0(n-x,n*q);

		/* f = (M_2PI*x*(n-x))/n; could overflow or underflow */
		/* Upto R 2.7.1:
		 * lf = log(M_2PI) + log(x) + log(n-x) - log(n);
		 * -- following is much better for  x << n : */
		lf = Constants.M_LN_2PI + log(x) + log1p(- x/n);
	...
	public static final double density(double x, double n, double p, boolean give_log)
	{
	    ...
	    if (p < 0 || p > 1 || (n < 0. || MathFunctions.isNonInt(n)))
			return Double.NaN;
	    //R_D_nonint_check(x);
	...
	{
		if (Double.isNaN(x) || Double.isNaN(n) || Double.isNaN(p)) return x + n + p;
		if (MathFunctions.isInfinite(n) || MathFunctions.isInfinite(p)) return Double.NaN;
		if(MathFunctions.isNonInt(n)) return Double.NaN;
		n = rint(n);
	...
	public static final double quantile(double p, double n, double pr, boolean lower_tail, boolean log_p)
	{
		...
		if(MathFunctions.isInfinite(n) || MathFunctions.isInfinite(pr)) return Double.NaN;
		/* if log_p is true, p = -Inf is a legitimate value */
		if(MathFunctions.isInfinite(p) && !log_p) return Double.NaN;
		...
		if (p + 1.01 * Constants.DBL_EPSILON >= 1.) return n;
	...
	public static final double random(double nin, double pp, RandomEngine random, RandomState state) {
		...
		if (MathFunctions.isInfinite(nin)) return Double.NaN;
		r = rint(nin); // floor(nin + 0.5);
		if (r != nin) return Double.NaN;
		if (MathFunctions.isInfinite(pp) ||
