...
public class Gamma extends GenericDistribution {
	static final double M_cutoff = Constants.M_LN2 * Constants.DBL_MAX_EXP / Constants.DBL_EPSILON;
	...
	static final double pgamma_smallx (double x, double alph, boolean lower_tail, boolean log_p)
	{
		...
		do {
			...
			term = c / (alph + n);
			sum += term;
		} while (abs (term) > Constants.DBL_EPSILON * abs (sum));
		...
		if (lower_tail) {
			...
			if (alph > 1) {
				...
				f2 = alph * log (x) - MathFunctions.lgamma1p (alph);
			else
				f2 = pow (x, alph) / exp (MathFunctions.lgamma1p (alph));
			return log_p ? f1 + f2 : f1 * f2;
		} else {
			double lf2 = alph * log (x) - MathFunctions.lgamma1p (alph);
			if (log_p) {
				lf2 = log1p (sum) + lf2;
				return lf2 > -Constants.M_LN2 ? log(-expm1(lf2)) : log1p(-exp(lf2));
			}
			else {
				double f1m1 = sum;
	...
	static final double dpnorm (double x, boolean lower_tail, double lp)
	{
		...
		if (x > 10 && !lower_tail) {
			...
			do {
				...
				sum += term;
				i += 2;
			} while (abs (term) > Constants.DBL_EPSILON * sum);
	...
	static final double ppois_asymp (double x, double lambda, boolean lower_tail, boolean log_p)
	{
		 ...
		 */
		pt_ = -MathFunctions.log1pmx (dfm / x);
		s2pt = sqrt (2 * x * pt_);
		if (dfm < 0) s2pt = -s2pt;
	...
	static final double pgamma_raw (double x, double alph, boolean lower_tail, boolean log_p)
	{
		...
		if (x < 1) {
			...
			if (!lower_tail) {
				if (log_p) {
					res = d + sum;
					res = res > -Constants.M_LN2 ? log(-expm1(res)) : log1p(-exp(res));
				} else res = 1 - d * sum;
			} else
			...
			double d = dpois_wrap (alph, x, log_p);
			if (alph < 1) {
				if (x * Constants.DBL_EPSILON > 1 - alph)
					sum = (log_p ? 0. : 1.);
				else {
		 ...
		 */
		if (!log_p && res < Constants.DBL_MIN / Constants.DBL_EPSILON) {
	...
	static final double qchisq_appr(double p, double nu, double g /* = log Gamma(nu/2) */,
		...
		p1 = (lower_tail? (log_p	? (p) : log(p)) : (log_p ? ((p) > -Constants.M_LN2 ? log(-expm1(p)) : log1p(-exp(p))) : log1p(-p)));
		if(nu < -1.24*p1) {	/* for small chi-squared */
			 ...
			 */
			double lgam1pa = (alpha < 0.5) ? MathFunctions.lgamma1p(alpha) : (log(alpha) + g);
			ch = exp((lgam1pa + p1)/alpha + Constants.M_LN2);
			...
			if( ch > 2.2*nu + 6 ) {
				...
				ch = -2*((lower_tail? (log_p ? (p) > -Constants.M_LN2 ? log(-expm1(p)) : log1p(-exp(p)) : log1p(-p)): (log_p ? (p) : log(p)))
						- c*log(0.5*ch) + g);
			}
			...
			a = (lower_tail? (log_p ? (p) > -Constants.M_LN2 ? log(-expm1(p)) : log1p(-exp(p)) : log1p(-p)): (log_p ? (p) : log(p)))
					+ g + c * Constants.M_LN2;
			do {
				q = ch;
				p1 = 1. / (1+ch*(C7+ch));
	...
	public static final double quantile(double p, double alpha, double scale, boolean lower_tail, boolean log_p)
	{
		...
		g = MathFunctions.lgammafn(alpha);/* log Gamma(v/2) */
			...
			if(ch < EPS2) {/* Corrected according to AS 91; MM, May 25, 1999 */
				...
				if(p_ > pMAX || p_ < pMIN) {
					...
					for(i=1; i <= MAXIT; i++ ) {
						...
						t = p2*exp(alpha * Constants.M_LN2+g+p1-c*log(ch));
						b = t/ch;
						a = 0.5*t - b*c;
