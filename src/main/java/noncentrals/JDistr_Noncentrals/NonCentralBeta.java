...
public class NonCentralBeta extends GenericDistribution {
	...
	public static final double cumulative_raw (double x, double o_x, double a, double b, double ncp) {
		...
		a0 = a + x0;
		lbeta = MathFunctions.lgammafn(a0) + MathFunctions.lgammafn(b) - MathFunctions.lgammafn(a0 + b);
		...
		gx = exp(a0 * log(x) + b * (x < .5 ? log1p(-x) : log(o_x))
				- lbeta - log(a0));
		if (a0 > a)
			q = exp(-c + x0 * log(c) - MathFunctions.lgammafn(x0 + 1.));
		else
			q = exp(-c);
		...
		if (errbd > errmax) {
			...
			if (Debug.WARNING_AS_ERROR) throw new PrecisionException("Precision error NonCentralBeta.cumulative", ans);
		}
		if (j >= itrmax + x0) {
			//ML_ERROR(ME_NOCONV, "pnbeta");
			System.err.println("Non-convergence error NonCentralBeta.cumulative");
			if (Debug.WARNING_AS_ERROR) throw new PrecisionException("Non-convergence error NonCentralBeta.cumulative", ans);
		}
	...
	/* o_x  == 1 - x  but maybe more accurate */
	{
		...
		if(ans > 1 - 1e-10) {
			// ML_ERROR(ME_PRECISION, "pnbeta");
			System.err.println("Precision error NonCentralBeta.cumulative");
			if (Debug.WARNING_AS_ERROR) throw new PrecisionException("Precision error NonCentralBeta.cumulative", log_p ? log1p(-min(ans, 1.0)): (1 - ans));
		}
	...
	public static final double quantile(double p, double a, double b, double ncp, boolean lower_tail, boolean log_p) {
		...
		if(p > 1 - Constants.DBL_EPSILON) return 1.0;
		pp = min(1 - Constants.DBL_EPSILON, p * (1 + Eps));
		for(ux = 0.5; ux < 1 - Constants.DBL_EPSILON && cumulative(ux, a, b, ncp, true, false) < pp; ux = 0.5*(1+ux)) ;
		pp = p * (1 - Eps);
		for(lx = 0.5; lx > Constants.DBL_MIN && cumulative(lx, a, b, ncp, true, false) > pp; lx *= 0.5) ;
	...
	*/
	static final double patnaik2(double x, double nu1, double nu2, double lambda) {
		return FDistribution.cumulative( x/(1+lambda/nu1), (nu1+lambda)*((nu1+lambda)/(nu1+2*lambda)), nu2, true, false);
	}
	...
	static final double guess(double prob, double y, double nu1, double nu2) {
		...
		lambdal = 0.0;
		lambdau = 1.0;
		fl = FDistribution.cumulative(x, nu1, nu2, true, false);
	...
	*/
	public static final double calculate_ncp(double prob, double x, double a, double b) {
		...
		eps  = Constants.DBL_EPSILON; //1.0e-7;
		eps2 = Constants.DBL_EPSILON; //1.0e-6;
