...
import noncentrals.JDistr_MathyStuff.*;
...
public class Beta extends GenericDistribution {
	...
	public static final double density(double x, double a, double b, boolean log_p)
	{
	    ...
	    if(a == 0 || b == 0 || MathFunctions.isInfinite(a) || MathFunctions.isInfinite(b)) {
	    ...
	    if (a <= 2 || b <= 2)
		lval = (a-1)*log(x) + (b-1)*log1p(-x) - MathFunctions.lbeta(a, b);
	    else
	...
	public static final double cumulative_raw(double x, double a, double b, boolean lower_tail, boolean log_p)
	{
		...
		// NB:  0 < x < 1 :
		if(a == 0 && b == 0) // point mass 1/2 at each of {0,1} :
		    return (log_p ? -Constants.M_LN2 : 0.5);
	    ...
	    int ierr;
	    double[] temp = MathFunctions.bratio(a, b, x, x1, log_p); /* -> ./toms708.c */
	...
	public static final double quantile (double alpha, double p, double q, boolean lower_tail, boolean log_p)
	{
		...
		double DBL_very_MIN  = Constants.DBL_MIN / 4.,
			DBL_log_v_MIN = Constants.M_LN2*(Constants.DBL_MIN_EXP - 2),
	    ...
	    if (p == 0 || q == 0 || MathFunctions.isInfinite(p) || MathFunctions.isInfinite(q)) {
	    	if(p == 0 && q == 0) { // point mass 1/2 at each of {0,1} :
	    	    if(alpha < (log_p ? -Constants.M_LN2 : 0.5)) return give_log_q ? Double.NEGATIVE_INFINITY : 0;
	    	    if(alpha > (log_p ? -Constants.M_LN2 : 0.5)) return give_log_q ? 0 : 1;
	    	    // else:  alpha == "1/2"
	    	    return give_log_q ? -Constants.M_LN2 : 0.5;
		...
		logbeta = MathFunctions.lbeta(p, q);
		...
		if(swap_tail) { /* change tail, swap  p <-> q :*/
			...
			la = lower_tail ? (log_p ? ((alpha) > -Constants.M_LN2 ? log(-expm1(alpha)) : log1p(-exp(alpha))) : log1p(-alpha)) : (log_p ? (alpha) : log(alpha));
			pp = q; qq = p;
		}
		else {
			a = p_;
			//la = lower_tail ? R_D_log(alpha) : R_D_LExp(alpha);
			la = lower_tail ? (log_p ? (alpha) : log(alpha)) : (log_p ? ((alpha) > -Constants.M_LN2 ? log(-expm1(alpha)) : log1p(-exp(alpha))) : log1p(-alpha));
			pp = p; qq = q;
		}
	    ...
	    */
		...
		final double
		log_eps_c = Constants.M_LN2 * (1. - Constants.DBL_MANT_DIG);// = log(DBL_EPSILON) = -36.04..
		r = pp*(1.-qq)/(pp+1.);
		...
		if(Constants.M_LN2 * Constants.DBL_MIN_EXP < u0 && // cannot allow exp(u0) = 0 ==> exp(u1) = exp(u0) = 0
				...
				u0 < (t*log_eps_c - log(abs(pp*(1.-qq)*(2.-qq)/(2.*(pp+2.)))))/2.)
		{
			...
			if (pp > 1 && qq > 1) { // use  Carter(1947), see AS 109, remark '5.'
				...
				if (t == 0 || (t < 0. && s >= t)) { // cannot use chisq approx
					...
					if(swap_tail)
						...
						l1ma = lower_tail ? (log_p ? (alpha) : log(alpha)) : (log_p ? ((alpha) > -Constants.M_LN2 ? log(-expm1(alpha)) : log1p(-exp(alpha))) : log1p(-alpha));
					else
						//l1ma = lower_tail ? R_D_LExp(alpha) : R_D_log(alpha);
						l1ma = lower_tail ? (log_p ? ((alpha) > -Constants.M_LN2 ? log(-expm1(alpha)) : log1p(-exp(alpha))) : log1p(-alpha)) : (log_p ? (alpha) : log(alpha));
					//R_ifDEBUG_printf(" t <= 0 : log1p(-a)=%.15g, better l1ma=%.15g\n", log1p(-a), l1ma);
					double xx = (l1ma + log(qq) + logbeta) / qq;
					if(xx <= 0.) {
						xinbta = -expm1(xx);
						//u = R_Log1_Exp (xx);// =  log(xinbta) = log(1 - exp(...A...))
						u = ((xx) > -Constants.M_LN2 ? log(-expm1(xx)) : log1p(-exp(xx)));// =  log(xinbta) = log(1 - exp(...A...))
			...
			if(swap_choose &&
				...
				//u = R_Log1_Exp(u);
				u = ((u) > -Constants.M_LN2 ? log(-expm1(u)) : log1p(-exp(u)));
				xinbta = exp(u);
			...
			if (!skip_to_return) {
				...
				if (bad_init && !(use_log_x && tx > 0)) {
					if(u == Double.NEGATIVE_INFINITY) {
						...
						u = Constants.M_LN2 * Constants.DBL_MIN_EXP;
						xinbta = Constants.DBL_MIN;
					} else {
		...
		if (!skip_to_return) {
			...
			if(use_log_x) { // find  log(xinbta) -- work in  u := log(x) scale
				...
				for (i_pb=0; i_pb < 1000; i_pb++) {
					...
					w = (y == Double.NEGATIVE_INFINITY) // y = -Inf  well possible: we are on log scale!
						? 0. : (y - la) * exp(y - u + logbeta + r * u + t * ((u) > -Constants.M_LN2 ? log(-expm1(u)) : log1p(-exp(u))));
				...
				for (i_pb=0; i_pb < 1000; i_pb++) {
					...
					{ // ML_ERR_return_NAN :
						...
						if (Debug.WARNING_AS_ERROR) throw new RuntimeException("MEDOMAIN on Beta.quantile");
						//qb[0] = qb[1] = ML_NAN; return;
						return Double.NaN;
					}
			...
			if((log_ && y == Double.NEGATIVE_INFINITY) || (!log_ && y == 0)) {
				...
				if (Debug.WARNING_AS_ERROR) throw new RuntimeException(errstr);
				}
			}
		}
		...
		if(give_log_q) { // ==> use_log_x , too
			...
			r = ((u_n) > -Constants.M_LN2 ? log(-expm1(u_n)) : log1p(-exp(u_n)));
			if(swap_tail) {
				qb[0] = r;	 qb[1] = u_n;
			} else {
				qb[0] = u_n; qb[1] = r;
	...
	{
		final double expmax = (Constants.DBL_MAX_EXP * Constants.M_LN2);
		double a, b, alpha;
		...
		if (MathFunctions.isInfinite(aa) && MathFunctions.isInfinite(bb))
			return 0.5;
		if (aa == 0. && bb == 0.)
			return (random.nextDouble() < 0.5) ? 0. : 1.;

		if (MathFunctions.isInfinite(aa) || bb == 0.)
			return 1.0;

		if (MathFunctions.isInfinite(bb) || aa == 0.)
			return 0.0;
			...
			for(;;) {
				...
				if (u1 < 0.5) {
					...
					if (z <= 0.25) {
						...
						if (v <= expmax) {
							w = b * exp(v);
							if(MathFunctions.isInfinite(w)) w = Constants.DBL_MAX;
						} else
							w = Constants.DBL_MAX;
						break;
					}
