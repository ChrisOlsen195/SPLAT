...
***************************************************************************/
    ...
    int nGroups, df, /*nUniques,*/ totalNSuccesses, totalNObservations,
        totalNFailures; //, nDiagnosticsLines;
    
    final int ONE = 1;
    //final int NVARIABLES = 1;
    final int NVARPLUS1 = 2;

    int[] successes_IthGroup, failures_IthGroup, totals_IthGroup, sortKey;
    
    double zBeta0, zBeta1, /*pBeta1,*/ oddsRatio, ciLower, ciUpper, pValuePearson,
           /*ssError,*/ pearsonChiSquare, beta0, beta1, seBeta0, 
           seBeta1, covBetas, deviance; // pValueDeviance, devianceChiSquare,
           //hh_Chi_Square;
    
    double llrm_1, llrm_2, llrm_3, /*lrt_1, lrt_2,*/ lrts_1, lrts_2, logLikeReduced, 
           logLikeFull, G_Statistic, LRTS_PValue, ithXValue;
    
    double [] estPi, fleissPHatX, fleissWi, originalProps, 
              logitProps, expectedSuccesses, theXValues, //propThisValue, 
              expectedProps, dbl_nTotals, sortedXValues; 
       
    String respVsExplanVar, xAxisLabel, format_02, format_01;
    String[] unique_Xs, grpResponseTable; //, formatSpecification;
    
    // String waldoFile = "MLR_Model";
    String waldoFile = "";
    ...
    Matrix hh_XTranspose, hh_XTrans_W_X, hh_XTrans_W_X_Inverse, hh_XTrans_W_Z,
           hh_Eta, hh_Z, mat_X, mat_XTranspose, /*mat_Y, mat_Z,*/ mat_Beta,
           mat_OrigProps, mat_LogitProps, devianceResiduals, pearsonResiduals, 
           standPearsonResiduals, estProbs, hh_W, mat_V;
    
    Matrix V_oneHalf, V_oneHalf_X, oneHalf_X, XTrans_V_X, XTrans_V_X_Inverse,
           XTrans_V_oneHalf, hatMatrix;
    
    QuantitativeDataVariable qdv_DevResids, qdv_PearsonResids;
    
    ArrayList<String> logisticReport, logisticDiagnostics;

    MLR_Controller mlr_Controller;
    //Logistic_View logRegView;
    
    TableFormatter tf;

    public MLR_Model(MLR_Controller mlr_Controller){
        this.mlr_Controller = mlr_Controller;
        dm = mlr_Controller.getDataManager();
        dm.whereIsWaldo(82, waldoFile, "Constructing");
        nGroups = mlr_Controller.getNUniqueXs();
        ...
        //propThisValue = new double[nGroups];
        ...
        respVsExplanVar = mlr_Controller.getRespVsExplSubtitle();
    ...
    public void doAllThatMathStuff() {        
        dm.whereIsWaldo(122, waldoFile, "doAllThatMathStuff()");
   ...
   private void doMoreIntitializations() {
        dm.whereIsWaldo(230, waldoFile, "doMoreIntitializations()");       
        unique_Xs = mlr_Controller.getUniqueXs();
        successes_IthGroup = mlr_Controller.getNSuccesses();
        totals_IthGroup = mlr_Controller.getNTotals();

        totalNSuccesses = mlr_Controller.getTotalNSuccesses();
        totalNObservations = mlr_Controller.getTotalNObservations();
        totalNFailures = mlr_Controller.getTotalNFailures();
   ...
   private void calculateDeviance() {
        dm.whereIsWaldo(345, waldoFile, "calculateDeviance()"); 
        //  ******************    Deviance   ***************************
        //  ******************    MPV, p432  ***************************
        ...
        for (int ithGroup = 0; ithGroup < nGroups; ithGroup++) {
            ...
            if (nSucc == 0) { tempLeft = 0; }
                ...
                tempLeft = nSucc * Math.log(nSucc / (nObs * piHat));
            }
    ...
    private void calculateResiduals() {
        ...
        dm.whereIsWaldo(376, waldoFile, "calculateResiduals()");        
        ...
        qdv_PearsonResids = new QuantitativeDataVariable(dm, "PearsonResids", "PearsonResids", pearsonResiduals); 
}
    
    private void doHatStuff() {
        dm.whereIsWaldo(376, waldoFile, "doHatStuff()");
        ...
        oneHalf_X = new Matrix(nGroups, nGroups);
    ...
    private void printStatistics() {
        ...
        double[] seBetasCol = new double[2];
        ...
        format_02 = tf.doTheFormatting(formatSpec_03);

        logisticReport.add(String.format(format_02, grpResponseTable[0], 
                                                     beta1 , seBeta1,  
   ...
   private void printDiagnostics() {
       ...
       addNBlankLinesToDiagnosticReport(2);
        ...
        //nDiagnosticsLines = logisticDiagnostics.size();
   }
   ...
   public MLR_Controller getLogisticProcedure() {return mlr_Controller; }
