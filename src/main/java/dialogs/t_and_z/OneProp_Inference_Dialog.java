...
public class OneProp_Inference_Dialog extends Splat_Dialog { 
    
    // POJOs
    boolean dlg_okToContinue; //, ok, valuesLeftBlank;
    boolean bool_PropGood, bool_SuccGood, bool_NGood;
    boolean allFieldsGood;
    
    int succ, n, alphaIndex, ciIndex;
    double prop, significanceLevel; //, confidenceLevel;
    ...
    private void makeNumericValuesPanel() {
        ...
        //stf_Prop.setSmartTextField_MB_NONBLANK(false);
        //al_stfForEntry.add(stf_Prop);
        
        al_STF.get(0).getTextField().setOnAction(e -> {            
            checkAndHandlelProportionEntered(al_STF.get(0));
        });
       
        //stf_Succ = new SmartTextField(propNStuffHandler, 2, 2);
        ...
        //stf_N = new SmartTextField(propNStuffHandler, 0, 1);
