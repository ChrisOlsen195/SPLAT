...
public class LinReg_Power_VsSampleSizeView extends BivariateScale_W_CheckBoxes_View
{
    // POJOs
    //boolean printTheStuff = true;
    boolean printTheStuff = false;
    int maxSampleSize;
    double yMin, yMax, effectSize, msResid, sigmaX, sampleSize;
    ...
    // My classes
    LinReg_Power_Model linReg_Power_Model;    // parent is NOT the Dashboard!!

    public LinReg_Power_VsSampleSizeView(LinReg_Power_Model linReg_Power_Model,
                         LinReg_Power_Dashboard linReg_Power_Dashboard,
                         double placeHoriz, double placeVert,
                         double withThisWidth, double withThisHeight) {        
        super(placeHoriz, placeVert, withThisWidth, withThisHeight);
        if (printTheStuff) {
            System.out.println("41 *** LinReg_Power_VsSampleSizeView, Constructing");
            System.out.println("42 *** LinReg_Power_VsSampleSizeView, Constructing");
            System.out.println("43 *** LinReg_Power_VsSampleSizeView, Constructing");
            System.out.println("44 *** LinReg_Power_VsSampleSizeView, Constructing");
            System.out.println("45 *** LinReg_Power_VsSampleSizeView, Constructing");
        }
        initHoriz = placeHoriz; initVert = placeVert;
        initWidth = withThisWidth; initHeight = withThisHeight; 
        this.linReg_Power_Model = linReg_Power_Model;
        // Max sample size is the maximum feasible sample size in practice
        maxSampleSize = linReg_Power_Model.getMaxSampleSize();
        ...
        fromHere = 1.0; toThere = maxSampleSize + 1;
        makeItHappen();
    }  
    ...
    public void doTheGraph() {      
        ...
        double forRestorationNullStErr = linReg_Power_Model.getStErr_Beta();
        double forRestorationAltStErr = linReg_Power_Model.getStErr_AltBeta();

        for (int daN = 2; daN < maxSampleSize + 1; daN++) {
        if (printTheStuff) {
            System.out.println("181 ... LinReg_Power_VsSampleSizeView, daN = " + daN);
        }
            dbl_daN = daN;  //  Sample size -- double
            linReg_Power_Model.setSampleSize(daN);

            // **********  Calculate changing stErrors ********** 
            msResid = linReg_Power_Model.getMSResid();
            sigmaX = linReg_Power_Model.getSigmaX();
            double temp_dbl_numerator = msResid;
            double temp_dbl_Denominator = (dbl_daN - 1.0) * sigmaX * sigmaX;
            double tempStErrNullBeta = Math.sqrt(temp_dbl_numerator / temp_dbl_Denominator );
            double tempStErrAltBeta = tempStErrNullBeta;
            linReg_Power_Model.setStErr_Beta(tempStErrNullBeta);
            linReg_Power_Model.setStErr_AltBeta(tempStErrNullBeta);
            power = linReg_Power_Model.calculatePower();    
            if (printTheStuff) {
                System.out.println("\n195 ... LinReg_Power_VsSampleSizeView, daN = " + daN);
                System.out.println("196 ... LinReg_Power_VsSampleSizeView, msResid = " + msResid);
                System.out.println("197 ... LinReg_Power_VsSampleSizeView, sigmaX = " + sigmaX);
                System.out.println("198 ... LinReg_Power_VsSampleSizeView, tempStErrNullBeta = " + tempStErrNullBeta);
                System.out.println("199 ... LinReg_Power_VsSampleSizeView, power = " + power);
                System.out.println("\n");
            }
        ...
        linReg_Power_Model.setStErr_Beta(forRestorationNullStErr);
