...
public class ANOVA2_MainEffect_AView extends ANOVA2_Views_Super { 
    
    // POJOs
    double daMiddleXPosition;
    
    String waldoFile = "";
    //String waldoFile = "ANOVA2_MainEffect_AView";
    
    // My Classes
    ANOVA2_Factorial_Model anova2_Factorial_Model;
    
    // FX Classes
    AnchorPane mainEffectA_AnchorPane;

    ANOVA2_MainEffect_AView(ANOVA2_Factorial_Model anova2_Factorial_Model, 
        ...
        nFactorA_Levels = anova2_Factorial_Model.str_ALevels.size();
        nFactorB_Levels = anova2_Factorial_Model.str_BLevels.size();
        ...
        preStrTopLabels = anova2_Factorial_Model.getFactorALevels();
        preStrLeftLabels = anova2_Factorial_Model.getFactorBLevels();
        ...
        factorA_Levels.addAll(anova2_Factorial_Model.getFactorALevels());
        factorB_Levels.addAll(anova2_Factorial_Model.getFactorBLevels()); 
    }
    ...
    public void doThePlot() {
        ...
        gc.setLineWidth(2);
        tempPos1 = categoryAxis_X.getDisplayPosition(factorA_Levels.get(1));      
        tempPos0 = categoryAxis_X.getDisplayPosition(factorA_Levels.get(0));      
        ...
        for (int theBetweenBatch = 1; theBetweenBatch < nFactorA_Levels; theBetweenBatch++) {
            daMiddleXPosition = categoryAxis_X.getDisplayPosition(factorA_Levels.get(theBetweenBatch));
            int theAppropriateLevel = theBetweenBatch;
            tempUCDO = anova2_Factorial_Model.getPrelimA().getIthUCDO(theAppropriateLevel);   
    ...
    private void initializeGraphParams() { 
        ...
        ****************************************************************/
        ...
        strTitle1 = "Main effect: " + anova2_Factorial_Model.getFactorALabel();
        ...
        for (int levelsA = 1; levelsA < nFactorA_Levels; levelsA++) {
            tempUCDO = anova2_Factorial_Model.getPrelimA().getIthUCDO(levelsA);
        ...
        for (int levelsB = 1; levelsB < nFactorB_Levels; levelsB++) {
            tempUCDO = anova2_Factorial_Model.getPrelimB().getIthUCDO(levelsB);
            double tempMean = tempUCDO.getTheMean();
            initial_yMin = Math.min(tempMean, initial_yMin);
            initial_yMax = Math.max(tempMean, initial_yMax);
        }        
