...
public class OneMean_Power_PdfView extends BivariateScale_W_CheckBoxes_View { 
    ...
    double yMin, yMax, scaleDelta, nullDiffMeans, altDiffMeans,
           ...
           yStop_Alt, standErr, effectSize, lowerSliver, upperSliver, 
           densityFactor;
    
    double rejectionArrowStart, rejectionArrowStop, rejectionFailureArrowStart, 
    ...
    double leftRejectionArrowStart, leftRejectionArrowStop,
    ...
    public OneMean_Power_PdfView(OneMean_Power_Model oneMean_Power_Model, 
        ...
        nullDiffMeans = oneMean_Power_Model.getNullMu();
        standErr = oneMean_Power_Model.getStandErrMean();        
        // Control the height of the normal curves
        densityFactor = 0.35 / Normal.density(nullDiffMeans, nullDiffMeans, standErr, false);
        ...
        nullDiffMeans = oneMean_Power_Model.getNullMu();
        rejectionCriterion = oneMean_Power_Model.getRejectionCriterion();
        
        switch (rejectionCriterion) {
            case "LessThan":
                altDiffMeans = nullDiffMeans - effectSize;
                break;
            ...
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 115 " + rejectionCriterion;
    ...
    public void makeTheCheckBoxes() {
        ...
        chBoxHBoxCheckBoxes = new CheckBox[nCheckBoxes];

        for (int i = 0; i < nCheckBoxes; i++) {
            chBoxHBoxCheckBoxes[i] = new CheckBox(strHBoxCheckBoxDescrs[i]);            
            chBoxHBoxCheckBoxes[i].setMaxWidth(Double.MAX_VALUE);
            chBoxHBoxCheckBoxes[i].setId(strHBoxCheckBoxDescrs[i]);
            chBoxHBoxCheckBoxes[i].setSelected(hBoxCheckBoxSettings[i]);

            chBoxHBoxCheckBoxes[i].setStyle(
                                "-fx-font-size: 14;" +
                                "-fx-border-insets: -5; " + 
                                "-fx-border-radius: 5;" +
            ...
            if (chBoxHBoxCheckBoxes[i].isSelected() == true) 
                chBoxHBoxCheckBoxes[i].setTextFill(Color.GREEN);
            else
                chBoxHBoxCheckBoxes[i].setTextFill(Color.RED);
            
            chBoxHBoxCheckBoxes[i].setOnAction(e->{
                ...
                switch (daID) {    
                    ...
                    default:
                        String switchFailure = "Switch failure: OneMean_Power_PdfView 193 " + daID;
                        MyAlerts.showUnexpectedErrorAlert(switchFailure);
                }
        ...
        checkBoxRow.getChildren().addAll(chBoxHBoxCheckBoxes);
    }
    ...
    public void setUpDecisionRegions() {
        ...
        switch(rejectionCriterion) {
            ...
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 239 " + rejectionCriterion;
                MyAlerts.showUnexpectedErrorAlert(switchFailure);    
        } 
    } 
    ...
    protected void setUpUI() { 
        String title1String = " Power, single Mean";
        String title2String = oneMean_Power_Model.getPrintedNullHypothesis() + " vs. " + oneMean_Power_Model.getPrintedAltHypothesis();
    ...
    public void setUpAnchorPane() {
        ...
        for (int iChex = 0; iChex < nCheckBoxes; iChex++) {            
            chBoxHBoxCheckBoxes[iChex].translateXProperty()
      ...
      public void initializeGraphParameters() {
        ...
        double lowZNull = nullDiffMeans - 3.5 * standErr;
        double hiZNull = nullDiffMeans + 3.5 * standErr; 
        double lowZAlt = altDiffMeans - 3.5 * standErr;
        double hiZAlt = altDiffMeans + 3.5 * standErr;
    ...
    public void doTheGraph() {      
        ...
        for (int chex = 0; chex < nCheckBoxes; chex++) {
            AnchorPane.setLeftAnchor(chBoxHBoxCheckBoxes[chex], (chex) * tempWidth / 5.0);
        }
        ...
        switch(rejectionCriterion) {
            ...
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 414 " + rejectionCriterion;
                MyAlerts.showUnexpectedErrorAlert(switchFailure); 
        }   //  end switch
        ...
        ************************************************************/
        xx0_Null = xGraphLeft; 
        yy0_Null = Normal.density(xx0_Null, nullDiffMeans, standErr, false); 
        for (double x = xGraphLeft; x <= xGraphRight; x += scaleDelta) {
            xx1_Null = x;
            density_Null = densityFactor * Normal.density(xx1_Null, nullDiffMeans, standErr, false); 
        ...
        ************************************************************/       
        if (!rejectionCriterion.equals("NotEqual")) {
            xx0_Alt = xGraphLeft; 
            yy0_Alt = Normal.density(xx0_Alt, altDiffMeans, standErr, false); 

            altScaleHeight = yAxis.getDisplayPosition(0.5);
            altScaleStart = xAxis.getDisplayPosition(nullDiffMeans - 3.5 * standErr);
            altScaleStop = xAxis.getDisplayPosition(altDiffMeans + 3.5 * standErr);
            gc.strokeLine(altScaleStart, altScaleHeight, altScaleStop, altScaleHeight); 

            for (double x = xGraphLeft; x <= xGraphRight; x += scaleDelta) {
                xx1_Alt = x;
                density_Alt = densityFactor * Normal.density(xx1_Alt, altDiffMeans, standErr, false);
        ...
        ************************************************************/
        ...
        for (double x = xGraphLeft; x <= xGraphRight; x += scaleDelta) {
            xx1_Null = x;
            density_Null = densityFactor * Normal.density(xx1_Null, nullDiffMeans, standErr, false);        
            density_Alt = densityFactor * Normal.density(xx1_Null, altDiffMeans, standErr, false);
            ...
            switch(rejectionCriterion) {
                ...
                default:
                    String switchFailure = "Switch failure: OneMean_Power_PdfView 563 " + rejectionCriterion;
                    MyAlerts.showUnexpectedErrorAlert(switchFailure); 
                    break;     
            }   //  end switch
        }  //   end loop   
        ...
        switch (rejectionCriterion) {
            case "LessThan":
                xCritValDescrStart = xAxis.getDisplayPosition(rawCritValLT - 0.75  * standErr);
                break;
                
            case "NotEqual":
                xCritValDescrStart = xAxis.getDisplayPosition(nullDiffMeans - 1.75  * standErr);
                break;
                
            case "GreaterThan":
                xCritValDescrStart = xAxis.getDisplayPosition(rawCritValGT - 0.75  * standErr);
                break;
                
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 586 " + rejectionCriterion;
                MyAlerts.showUnexpectedErrorAlert(switchFailure); 
                break; 
        } 
        ...
        switch (rejectionCriterion) {
            case "LessThan": 
                xAltDistDescr = xAxis.getDisplayPosition(nullDiffMeans + 1.75  * standErr);
                break;
            ...
            case "GreaterThan": 
                xAltDistDescr = xAxis.getDisplayPosition(nullDiffMeans - 3.0  * standErr);
                break;
                
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 612 " + rejectionCriterion;
                MyAlerts.showUnexpectedErrorAlert(switchFailure); 
                break; 
        }
        ...
        switch(rejectionCriterion) {
            case "LessThan":    // Alternative is less than        
                if (annotation_Desired) {
                    ...
                    rejectionArrowStop = xAxis.getDisplayPosition(rawCritValLT - 1.5 * standErr);

                    rejectionFailureArrowStart = rejectionArrowStart;
                    rejectionFailureArrowStop = xAxis.getDisplayPosition(rawCritValLT + 1.5 * standErr);
                    nonRejectionTextStart = xAxis.getDisplayPosition(standErr + 1.5 * standErr);
                    rejectionTextStart = xAxis.getDisplayPosition(rawCritValLT - 1.7 * standErr);
                    ...
                    nonRejectionTextStart = xAxis.getDisplayPosition(rawCritValLT + 2.0 * standErr);
                    
                    drawAnArrow(rawCritValLT, rawCritValLT - 1.5 * standErr, 0.40);
                    drawAnArrow(rawCritValLT, rawCritValLT + 1.5 * standErr, 0.40);
            ...
            case "NotEqual":    // Alternative is not equal to
                if (annotation_Desired ) {
                    ...
                    leftRejectionArrowStop = xAxis.getDisplayPosition(rawCritValLT - 1.5 * standErr);
                    rightRejectionArrowStart = xAxis.getDisplayPosition(rawCritValGT);
                    rightRejectionArrowStop = xAxis.getDisplayPosition(rawCritValGT + 1.5 * standErr);
                    
                    nonRejectionTextStart = xAxis.getDisplayPosition(nullDiffMeans - 1.7 * standErr);
                    ...
                    leftRejectionTextStart = xAxis.getDisplayPosition(rawCritValLT - 1.25 * standErr);
                    rightRejectionTextStart = xAxis.getDisplayPosition(rawCritValGT + 1.00 * standErr);
                    
                    gc.strokeLine(leftRejectionArrowStart, nullArrowHeight, leftRejectionArrowStop, nullArrowHeight); 
                    gc.strokeLine(rejectionFailureArrowStart, nullArrowHeight, rejectionFailureArrowStop, nullArrowHeight);
                    
                    xNotEqualNoRejectRegionStart = xAxis.getDisplayPosition(nullDiffMeans - 0.25 * standErr); 
                    ...
                    drawAnArrow(rawCritValLT, rawCritValLT - 1.5 * standErr, 0.40);                    
                    drawAnArrow(rawCritValGT, rawCritValGT + 1.5 * standErr, 0.40);
            ...
            case "GreaterThan":    // Alternative is greater than
                if (annotation_Desired) {
                    ...
                    rejectionArrowStop = xAxis.getDisplayPosition(rawCritValGT + 1.5 * standErr);

                    rejectionFailureArrowStart = rejectionArrowStart;
                    rejectionFailureArrowStop = xAxis.getDisplayPosition(rawCritValGT - 1.5 * standErr);
                    nonRejectionTextStart = xAxis.getDisplayPosition(nullDiffMeans - 2.7 * standErr);
                    rejectionTextStart = xAxis.getDisplayPosition(rawCritValGT + 1.0 * standErr);
                    gc.strokeLine(rejectionArrowStart, nullArrowHeight, rejectionArrowStop, nullArrowHeight); 
                    gc.strokeLine(rejectionFailureArrowStart, nullArrowHeight, rejectionFailureArrowStop, nullArrowHeight);
            ...
            default:
                String switchFailure = "Switch failure: OneMean_Power_PdfView 714 " + rejectionCriterion;
                MyAlerts.showUnexpectedErrorAlert(switchFailure); 
                break; 
        }
    ...
    private void drawAnArrow (double fromHere, double toThere, double atThisHeight) {
        ...
        horizontalArrowPointLength = 0.15 * standErr;
   ...
   ********************************************************************/
    ...
    public double[] getInverseMiddle_Z_Area(double middleArea)  {
        ...
        middleInterval[0] = Normal.quantile(lowerSliver, nullDiffMeans, standErr, true, false);
        middleInterval[1] = Normal.quantile(upperSliver, nullDiffMeans, standErr, true, false);
