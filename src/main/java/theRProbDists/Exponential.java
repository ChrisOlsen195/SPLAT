...
public class Exponential extends GenericDistribution {
	...
	public static final double cumulative(double x, double scale, boolean lower_tail, boolean log_p)
	{
		...
		if (lower_tail)
			return (log_p
					/* log(1 - exp(x))  for x < 0 : */
					? (x > -Constants.M_LN2 ? Math.log(-Math.expm1(x)) : Math.log1p(-Math.exp(x)))
							: -Math.expm1(x));
		/* else:  !lower_tail */
		//return R_D_exp(x);
	...
	public static final double quantile(double p, double scale, boolean lower_tail, boolean log_p)
	{
		...
		return - scale * (lower_tail? log_p ? ((p) > -Constants.M_LN2 ? Math.log(-Math.expm1(p)) : Math.log1p(-Math.exp(p))) : Math.log1p(-p): (log_p ? (p) : Math.log(p)));
	}
