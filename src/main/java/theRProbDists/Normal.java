...
public class Normal extends GenericDistribution {
    ...
    public double getRightArea (double z) {
        return 1.0 - cumulative(z);
    }
    
    public double getLeftArea (double z) {
        return cumulative(z);
    }
    
    public double getMeanZArea (double z) {
        return cumulative(Math.abs(z)) - 0.50;        
    }
    
    public double getTwoTails (double z) {
        return (1.0 - cumulative(Math.abs(z))) * 2.0;
    }
    
    public double getAreaBetweenZs (double z1, double z2) {
        
        double p = 0.0, start, end;
        if (z1 < z2) {
                start = z1;
                end = z2;
            } else if (z1 > z2) {
            ...
            double p1 = 1.0 - cumulative(Math.abs(start));
            double p2 = 1.0 - cumulative(Math.abs(end));
    ...
    public double getInvLeftArea (double p) {
        return quantile(p);
    }
    
    public double getInvRightArea (double p) {
        return quantile(1.0 - p);
    }
    
    public double getInvTwoTails (double p) {
        return quantile(1.0 - (p / 2.0));        
    }
    
    public double getPower (double zAlt, double direction, double alpha) {
        
        // direction: 0 = two-tails, +1 = alt z > 0, -1 alt z < 0
        
        double statPower = 0.00;
        
        if (direction == 0.0) {
            alpha = alpha / 2.0;
        }
        double zCrit = getInvRightArea(alpha);
        boolean correct = true;
               
        double zDiff = 0.0, zDiff2 = 0.0;
        if (zAlt > 0.0) {
            zDiff = zCrit - zAlt;
            statPower = getRightArea(zDiff);
            if (direction == 0.0) {
                zDiff2 = (zCrit * -1.0) - zAlt;
                statPower = statPower + getLeftArea(zDiff2);
            }
            if (direction == -1.0) {
                correct = false;
            }
        } else {
            zDiff = (zCrit * -1.0) - zAlt;
            statPower = getLeftArea(zDiff);            
            if (direction == 0.0) {
                zDiff2 = zCrit - zAlt;
                statPower = statPower + getRightArea(zDiff2);
            }
            if (direction == 1.0) {
                correct = false;
            }
	...
	public static final double density(double x, double mu, double sigma, boolean give_log) {
		...
		if (give_log)
			return -(Constants.M_LN_SQRT_2PI + 0.5 * x * x + log(sigma));
		if (x < 5) return Constants.M_1_SQRT_2PI * exp(-0.5 * x * x) / sigma;
	    /* ELSE:
	     ...
	     */
	    if (x > sqrt(-2 * Constants.M_LN2*(Constants.DBL_MIN_EXP + 1 - Constants.DBL_MANT_DIG))) return 0.;
	    ...
	    double x1 = //  R_forceint(x * 65536) / 65536 =
	    	MathFunctions.ldexp( rint(MathFunctions.ldexp(x, 16)), -16);
	    double x2 = x - x1;
	    return Constants.M_1_SQRT_2PI / sigma *
	...
	public static final double cumulative(double x, double mu, double sigma, boolean lower_tail, boolean log_p) {
		...
		if (y <= 0.67448975) { /* qnorm(3/4) = .6744.... -- earlier had 0.66291 */
			...
			xsq = MathFunctions.trunc(y * SIXTEN) / SIXTEN;
			del = (y - xsq) * (y + xsq);
			if (log_p) {
			...
			temp = (Constants.M_1_SQRT_2PI - temp) / y;
			/*!* 	xsq = trunc(x * SIXTEN) / SIXTEN; *!*/
			xsq = MathFunctions.trunc(x * SIXTEN) / SIXTEN;
			del = (x - xsq) * (x + xsq);
			if (log_p) {
				result = (-xsq * xsq * 0.5) + (-del * 0.5) + log(temp);
				if((lower_tail && x > 0.) || (!lower_tail && x <= 0.))
	...
	public static final double random_box_muller(double mu, double sigma, RandomEngine random) {
		double
			...
			R = sqrt(-2 * log(random.nextDouble())) + 10 * Constants.DBL_MIN;
