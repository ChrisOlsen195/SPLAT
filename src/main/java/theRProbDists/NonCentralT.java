...
import static theRProbDists.Constants.DBL_MIN_EXP;
import static theRProbDists.Constants.M_LN2;
import static theRProbDists.Constants.M_LN_SQRT_PI;
import static theRProbDists.Constants.M_SQRT_2dPI;
import static theRProbDists.MathFunctions.lgammafn;
...
public class NonCentralT extends GenericDistribution {
	...
	public static final double density(double x, double df, double ncp, boolean give_log) {
		...
		if(ncp == 0.0) return T_double_df.density(x, df, give_log);
	...
	public static final double cumulative(double t, double df, double ncp, boolean lower_tail, boolean log_p) {
		...
		if (df <= 0.0) return Double.NaN;
		if(ncp == 0.0) return T_double_df.cumulative(t, df, lower_tail, log_p);
	...
	public static final double quantile(double p, double df, double ncp, boolean lower_tail, boolean log_p) {
		...
		if(ncp == 0.0 && df >= 1.0) return T_double_df.quantile(p, df, lower_tail, log_p);
	...
	public static final double random(double df, double ncp, RandomEngine random) {
		if (ncp == 0)
			return T_double_df.random(df, random);
