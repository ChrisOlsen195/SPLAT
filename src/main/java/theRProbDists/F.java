...
public class F extends GenericDistribution {
    ...
    public double getPDF(double df1, double df2, double f) {
        return density(f, df1, df2, false);
    }

    public double getLeftArea(double df1, double df2, double f) {
        return cumulative(f, df1, df2, true, false);
    }

    public double getRightArea(double df1, double df2, double f) {
        return cumulative(f, df1, df2, false, false);
    }

    public double getInvLeftArea (double df1, double df2, double p) {
        return quantile(p, df1, df2, true, false);
    }
    
    public double getInvRightArea (double df1, double df2, double p) {
        return quantile(p, df1, df2, false, false);
    }
    ...
    public static final double density(double x, double m, double n, boolean give_log) {
        ...
        if (x == 0.) {
            return (m > 2 ? (give_log ? Double.NEGATIVE_INFINITY : 0.) : (m == 2 ? (give_log ? 0. : 1.) : Double.POSITIVE_INFINITY));
        }
